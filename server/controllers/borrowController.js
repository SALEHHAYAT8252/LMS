...
export const returnBorrowBook = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!borrow) {
    return next(new ErrorHandler("You have not borrow this book", 404));
  }

  // console.log(borrow.dueDate, Date.now());
  borrow.returnDate = Date.now(); // Set the return date to now
  const fine = calculateFine(borrow.dueDate);
  borrow.fine = fine; // Set the fine amount
  await borrow.save();
  res.status(200).json({
    success: true,
    message:
